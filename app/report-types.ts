...
export type ReportSector = {
  ...
  id: string; code: string; name: string; selectionScore: number; changePct: number; turnover: number;
...
export type DailyReport = {
  ...
  meta: { discovery?: { universeStocks: number; sampleStocks: number; tagCoverage: number; candidateTopics: number; eligibleTopics: number; amountCoverage: number; suppressed: {name: string; representedBy: string}[] }; version: string; analysisVersion?: string; tradeDate: string; previousDate: string; collectedAt: string; interactionAsOf: string; cutoff: string; selectionNote: string; marketSource: string; feedObserved: number; feedExpected: number; errors: { group: string; key: string; error: string }[]; flowNote: string };
