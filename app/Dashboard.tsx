...
import type { MetricKey, ViewKey, Sector, CommentRow, Correlation, CalendarPoint, Source, Snapshot } from "./types";
import { LineChart, HeatSparkline } from "./charts";
import { ScoreScale, ScoreBreakdown, EvidenceFeed, SectorResearchDialog, SectorCalendar, ResearchQA } from "./research";
...
const formatScore = (value: number | null | undefined, digits = 1) => value == null ? "—" : value.toFixed(digits);
...
function SectorCard({ sector, comment, onSelect }: { sector: Sector; comment?: CommentRow; onSelect: (sector: Sector) => void }) {
  const directionLabel = sector.direction == null ? "暂无" : sector.direction > 8 ? "偏多" : sector.direction < -8 ? "偏空" : "中性";
    ...
    <article className="sector-card">
      <div className="sector-topline">
        ...
        <div className="sector-score">
          <strong style={{ color: scoreBand(sector.overall).color }}>{formatScore(sector.overall, 0)}<small>/100</small></strong>
          <span className={(sector.change ?? 0) >= 0 ? "rise" : "fall"}>{signed(sector.change)}</span>
        </div>
      </div>
      <div className="sector-bar" aria-label={`${sector.name}温度 ${formatScore(sector.overall, 1)} 分`}>
      ...
      <div className="sector-hotline">
        ...
        <span>热度 {formatScore(sector.heat, 1)}</span>
      ...
      <div className="sector-detail-grid">
        <span><small>赚钱效应</small><b>{formatScore(sector.profitEffect, 1)}</b></span>
      ...
      {comment && <button className="card-evidence" onClick={() => onSelect(sector)}><span>{comment.intent} · 代表表达</span><q>{comment.excerpt}</q><small>{comment.source} · {comment.date}</small></button>}
      <button className="sector-research-button" onClick={() => onSelect(sector)}>查看评分依据与历史 ↗</button>
    </article>
  );
}
...
function CommentFeed({ comments }: { comments: CommentRow[] }) {
  return <EvidenceFeed comments={comments} />;
}

function MoodCalendar({ points }: { points: CalendarPoint[] }) {
  const bucketLabel = { cold: "冷清", mild: "温和", active: "活跃", hot: "高热", extreme: "极热" };
  return (
    <div>
      <div className="calendar-grid">
        {points.length ? points.map((point) => (
          <div className={`calendar-cell ${scoreBand(point.overall).key}`} key={point.date} title={`${point.date} · 温度 ${formatScore(point.overall, 1)} · 热度 ${formatScore(point.heat, 1)} · ${point.sampleCount == null ? "—" : `${point.sampleCount} 条`} · ${point.recordType === "estimated" ? "估算" : "实测"}`}>
            <strong>{point.date.slice(5)}</strong>
            <span>{formatScore(point.overall, 0)}</span>
          </div>
...
function QueryPanel({ snapshot, onSelect }: { snapshot: Snapshot; onSelect: (sector: Sector) => void }) {
  ...
  const changeQuery = (value: string) => {
    queryRequest.current += 1; queryController.current?.abort();
    setQuery(value); setOnlineResult(null); setOnlineError(""); setOnlineLoading(false);
  };
  const result = useMemo(() => {
    const keyword = query.trim().toLowerCase();
    if (!keyword) return null;
    return snapshot.sectors.find((sector) => [sector.id, sector.name, sector.stockCode, sector.representative, sector.group].some((value) => String(value ?? "").toLowerCase().includes(keyword))) ?? null;
  ...
  const runQuery = async (value = query) => {
    ...
    if (!keyword) return;
    const requestId = ++queryRequest.current;
    ...
    const local = snapshot.sectors.find((sector) => [sector.id, sector.name, sector.stockCode, sector.representative, sector.group].some((item) => String(item ?? "").toLowerCase().includes(keyword.toLowerCase())));
    if (local) return;
    setOnlineLoading(true);
    const controller = new AbortController();
    queryController.current = controller;
    try {
      const response = await fetch(`/api/stock-query?q=${encodeURIComponent(keyword)}`, { cache: "no-store", signal: controller.signal });
      ...
      if (requestId === queryRequest.current) setOnlineResult(payload);
    } catch (error) {
      setOnlineError(error instanceof Error ? error.message : "在线查询失败，请稍后重试");
    } finally {
      if (requestId === queryRequest.current) setOnlineLoading(false);
    }
  };
  ...
  return (
    <article className="panel query-panel">
      ...
      <div className="query-form"><input aria-label="输入股票代码、名称或主题" placeholder="例如：600519、贵州茅台、300750、宁德时代" value={query} onChange={(event) => changeQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void runQuery(); }} /><button onClick={() => void runQuery()} disabled={onlineLoading}>{onlineLoading ? "查询中…" : "查询"}</button></div>
      ...
      {result && !onlineResult && <div className="query-result"><div className="query-result-head"><div><span className="eyebrow">{result.group} · 代表标的 {result.representative}</span><h3>{result.name}</h3></div><strong>{formatScore(result.overall, 1)}</strong></div><div className="query-stat-grid"><span>热度 <b>{formatScore(result.heat, 1)}</b></span><span>买入 <b>{formatScore(result.buyIndex)}</b></span><span>卖出 <b>{formatScore(result.sellIndex)}</b></span><span>赚钱效应 <b>{formatScore(result.profitEffect, 1)}</b></span><span>涨跌 <b>{formatPct(result.priceChange)}</b></span><span>净流入 <b>{formatFlow(result.flowNet)}</b></span></div><HeatSparkline points={result.heatSeries ?? []} /><button className="sector-research-button" onClick={() => onSelect(result)}>查看板块评分依据与完整历史 ↗</button><CommentFeed comments={relatedComments} /></div>}
      {onlineResult && <div className="query-result online-query-result"><div className="query-result-head"><div><span className="eyebrow">{onlineResult.market} · {onlineResult.symbol} · 公开社区样本</span><h3>{onlineResult.name}</h3><small className="query-source">查询耗时 {onlineResult.durationMs}ms · {onlineResult.fetchedAt.slice(0, 16).replace("T", " ")}</small></div><strong>{onlineMetrics ? onlineMetrics.overall.toFixed(1) : "—"}</strong></div>{onlineMetrics ? <div className="query-stat-grid"><span>讨论热度 <b>{onlineMetrics.heat.toFixed(1)}</b></span><span>新手入场 <b>{onlineMetrics.novice.toFixed(1)}</b></span><span>追涨冲动 <b>{onlineMetrics.fomo.toFixed(1)}</b></span><span>恐慌割肉 <b>{onlineMetrics.panic.toFixed(1)}</b></span><span>买入指数 <b>{onlineMetrics.buyIndex.toFixed(1)}</b></span><span>卖出指数 <b>{onlineMetrics.sellIndex.toFixed(1)}</b></span><span>赚钱效应 <b>{onlineMetrics.profitEffect.toFixed(1)}</b></span><span>代表股涨跌 <b className={onlineResult.quote && onlineResult.quote.changePct >= 0 ? "rise" : "fall"}>{onlineResult.quote ? formatPct(onlineResult.quote.changePct) : "—"}</b></span></div> : <div className="query-empty">最近公开帖子不足，暂时无法计算情绪指数。</div>}{onlineResult.quote && <div className="query-quote"><b>{onlineResult.quote.price.toFixed(2)}</b><span className={onlineResult.quote.changePct >= 0 ? "rise" : "fall"}>{formatPct(onlineResult.quote.changePct)}</span><small>{onlineResult.quote.source} · {onlineResult.quote.asOf || "最新"}</small></div>}<div className="online-post-head"><span className="eyebrow">最近典型帖子</span><small>原始 {onlineResult.sampleCount} 条 · 有效分析 {onlineResult.analyzedCount} 条</small></div><EvidenceFeed comments={onlineResult.posts.map((post, index) => ({ id: `${post.url}-${index}`, date: post.date, source: "东方财富股吧", sectorId: onlineResult.code, sectorName: onlineResult.name, excerpt: post.title, tone: "neutral", intent: "表达样本", signals: post.matched, score: post.score, reasoning: post.reasoning, evidence: post.evidence, url: post.url }))} />{onlineResult.fetchError && <p role="status" className="query-error">{onlineResult.fetchError}</p>}<ScoreScale value={onlineMetrics?.overall} />{onlineMetrics && <ScoreBreakdown metrics={onlineMetrics} version={onlineResult.methodVersion ?? "旧版在线查询"} />}<p className="online-note">{onlineResult.dataWindow} · 置信 {onlineResult.confidence ?? "—"}。公式与每日快照一致，采样时间和来源覆盖不同，分数可能不同。{onlineResult.note}</p></div>}
...
function IntelligencePanel({ snapshot }: { snapshot: Snapshot }) {
  return (
    <section className="intelligence-grid">
      <article className="panel insight-card"><div className="panel-head"><div><span className="eyebrow">快照解读 · 本地规则</span><h2>今天市场怎么吵</h2></div><span className="pulse-dot" /></div><p>{snapshot.interpretation?.interpretation ?? "等待更多数据后生成解读。"}</p><small>解读引用当前快照的热度、情绪、样本与板块变化，不调用外部模型。</small></article>
...
export function Dashboard({ initialSnapshot }: { initialSnapshot: Snapshot }) {
  ...
  const history = useMemo(() => (legacyHistory ? snapshot.legacy?.history ?? [] : snapshot.history).slice(-range), [snapshot.history, snapshot.legacy, range, legacyHistory]);
  ...
  const rankedSectors = useMemo(() => {
    ...
    if (sectorSort === "heatChange") return sectors.sort((a, b) => b.heatChange - a.heatChange || (b.heat ?? -1) - (a.heat ?? -1));
    if (sectorSort === "heat") return sectors.sort((a, b) => (b.heat ?? -1) - (a.heat ?? -1) || (b.overall ?? -1) - (a.overall ?? -1));
    if (sectorSort === "flow") return sectors.sort((a, b) => (b.flowNet ?? -Infinity) - (a.flowNet ?? -Infinity));
    return sectors.sort((a, b) => (b.profitEffect ?? -1) - (a.profitEffect ?? -1) || (b.overall ?? -1) - (a.overall ?? -1));
  }, [snapshot.sectors, sectorGroup, sectorSort]);
  const hotSectors = useMemo(() => [...snapshot.sectors].sort((a, b) => b.heatChange - a.heatChange || (b.heat ?? -1) - (a.heat ?? -1)).slice(0, 4), [snapshot.sectors]);
  ...
  return (
    <main>
      {selectedSector && <SectorResearchDialog key={`${selectedSector.id}-${snapshot.meta.generatedAt}`} sector={snapshot.sectors.find(sector => sector.id === selectedSector.id) ?? selectedSector} snapshot={snapshot} onClose={() => setSelectedSector(null)} />}
      ...
      {view === "overview" && (
        <>
          <section className="hero" id="top">
            ...
            <div className="score-panel">
              ...
              <div className="score-copy">
                ...
                <h2>{snapshot.summary.label}</h2><ScoreScale value={snapshot.summary.overall} />
                <p>{snapshot.summary.readout}</p>
                <div className="score-change">
                  <b className={(snapshot.summary.change ?? 0) >= 0 ? "rise" : "fall"}>{signed(snapshot.summary.change)}</b>
                  <span>{snapshot.summary.change == null ? "新公式基线积累中" : "较前次同公式观测"}</span>
                </div>
              </div>
            </div>
          ...
          <section className="metrics-grid" aria-label="情绪分项">
            {METRICS.map((metric) => <MetricCard key={metric.key} label={metric.label} hint={metric.hint} color={metric.color} value={snapshot.summary[metric.key]} />)}
          </section>

          {marketStats && (
            <section className="market-strip" aria-label="市场赚钱效应与资金流向">
              <MarketStatCard label="全市场热度" value={formatScore(marketStats.heat, 1)} hint="社区讨论活跃度" tone="rise" />
              <MarketStatCard label="代表池赚钱效应" value={formatScore(marketStats.profitEffect, 1)} hint={`${marketStats.breadthUp ?? 0} 涨 / ${marketStats.breadthDown ?? 0} 跌 · 中位 ${formatPct(marketStats.breadthMedianChange)}`} />
          ...
          <section className="content-grid">
            <article className="panel trend-panel">
              ...
              {snapshot.legacy?.history?.length ? <label className="legacy-toggle"><input type="checkbox" checked={legacyHistory} onChange={event => setLegacyHistory(event.target.checked)} />查看旧版参考曲线（{snapshot.legacy.methodVersion}，与新公式分开）</label> : null}
              <LineChart points={history} metric={chartMetric} />
              <p className="history-note"><span className="history-key estimated" />已显示 {history.length}/{range} 个有观测交易日。{legacyHistory ? "旧版参考曲线，不与新公式分数直接比较。" : snapshot.meta.historyNote ?? "历史曲线由每日实测逐步累积。"}</p>
            </article>
          ...
          <section className="panel sectors-preview">
            ...
            <div className="sector-grid">{hotSectors.map((sector) => <SectorCard key={sector.id} sector={sector} comment={snapshot.comments?.find(comment => comment.sectorId === sector.id)} onSelect={setSelectedSector} />)}</div>
          </section>
          <IntelligencePanel snapshot={snapshot} />
          <section className="content-grid secondary-grid">
            <article className="panel"><div className="panel-head"><div><span className="eyebrow">匿名评论样本</span><h2>今天大家在说什么</h2></div><button className="text-button" onClick={() => setView("explore")}>查看更多 →</button></div><CommentFeed comments={(snapshot.comments ?? []).slice(0, 8)} /></article>
        ...
        <section className="view-page">
          ...
          {marketStats && (
            <section className="market-strip sector-market-strip" aria-label="全市场概览">
              <MarketStatCard label="市场热度" value={formatScore(marketStats.heat, 1)} hint="社区讨论活跃度" tone="rise" />
              <MarketStatCard label="代表池赚钱效应" value={formatScore(marketStats.profitEffect, 1)} hint={`${marketStats.breadthUp ?? 0} 涨 / ${marketStats.breadthDown ?? 0} 跌`} />
          ...
          <div className="sector-grid full">{rankedSectors.map((sector) => <SectorCard key={sector.id} sector={sector} comment={snapshot.comments?.find(comment => comment.sectorId === sector.id)} onSelect={setSelectedSector} />)}</div>
        </section>
      )}
      ...
      {view === "explore" && (
        <section className="view-page">
          ...
          <QueryPanel snapshot={snapshot} onSelect={setSelectedSector} />
          <section className="panel explore-comments"><div className="panel-head"><div><span className="eyebrow">快照评论样本</span><h2>匿名表达样本</h2></div><span className="eyebrow">{(snapshot.comments ?? []).length} 条</span></div><CommentFeed comments={snapshot.comments ?? []} /></section>
        </section>
      )}
      ...
      {view === "intelligence" && (
        <section className="view-page">
          ...
          <IntelligencePanel snapshot={snapshot} />
          <ResearchQA snapshot={snapshot} onSelect={setSelectedSector} />
          <SectorCalendar snapshot={snapshot} onSelect={setSelectedSector} />
      ...
      {view === "method" && (
        <section className="view-page">
          ...
          <ScoreBreakdown metrics={snapshot.summary} version={snapshot.meta.methodVersion} />
          <div className="audit-grid">
            ...
            <article className="panel method-card">
              ...
              <p>每个来源先独立聚合，再等权合并。V3 只合并有样本的来源；缺失来源不计分，并降低置信度。无样本显示“—”。</p>
            </article>
            <article className="panel method-card">
              <span className="eyebrow">03 · 隐私</span><h2>聚合指标与匿名证据</h2>
